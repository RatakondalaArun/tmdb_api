...
void main() {
  ...
  group('Movie', () {
    test('Details', () async {
      final result = await tmdb.v3.movies.getDetails(103, language: 'en-US');
    ...
    group('Latest', () {
      test('with parameters', () async {
        final result = await tmdb.v3.movies.getLatest(language: 'en-US');
        expect(result is Map, true, reason: 'result is not map');
        expect(result.containsKey('status_code'), false,
            reason:
                'error in api, contains status_code= ${result['status_code']}, status_message= ${result['status_message']}');
    ...
    group('>Rate Movie', () {
      test('>check', () async {
        final result = await tmdb.v3.movies.rateMovie(12, 5,
    ...
    group('>Delete Movie Rating', () {
      test('>check', () async {
        final result = await tmdb.v3.movies.deleteRating(5,
            sessionId: '2e900a73d597f46bb2abb9663adcabe05d5204f6');
        expect(result is Map, true, reason: 'result is not map');
      });
    });
    ...
    group('>account status', () {
      test('>check', () async {
        final result = await tmdb.v3.movies.getAccountStatus(12,
